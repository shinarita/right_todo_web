...
import {
  ...
  Switch, Button, FormLabel, FormHelperText, Checkbox, FormGroup, Select, MenuItem, CircularProgress
...
import { putRequest } from '../../../../middlewares/request'
...
const FormSchema = Yup.object().shape({
  ...
  startDate: Yup.date().min(new Date(1900, 0, 1)).nullable(),
...
export default function Edit({ history, match }) {
  ...
  useEffect(() => {
    document.title = 'Edit-RightTodo'
  }, [])

  const [{ data, isGetting, isError }] = useGetDataApi(
    { url: `todo/item/${id}` },
    initialData,
    [id]
  )
  ...
  const { title, description, priority, startDate, endDate, type, dates } = data
  const handleSubmit = (values, formikBag) => {
    ...
    putRequest(`todo/item/${id}`, todoItem)
      .then(res => {
  ...
  const initialValues = { title, description, priority, onlyToday: startDate === endDate, startDate, endDate, type, dates }
  return (
    <Paper className={classes.paper}>
      <Formik
                        ...
                        <Grid container alignItems='baseline' >
                          ...
                          <Grid item xs={12} sm={10}>
                            <RadioGroup name='type' value={values.type} onChange={handleChangeType} row >
                              <FormControlLabel value='day' control={<Radio />} label='每天' disabled />
                              <FormControlLabel value='week' control={<Radio />} label='每周' disabled />
                              <FormControlLabel value='month' control={<Radio />} label='每月' disabled />
                            </RadioGroup>
                            <FormHelperText>{endDateError && errors.endDate}</FormHelperText>
                          </Grid>
                        </Grid>
                                  ...
                                  <FormGroup row>
                                    {
                                      WeekDays.map(item => {
                                        ...
                                        return (
                                          <FormControlLabel
                                            key={value}
                                            control={<Checkbox disabled checked={values.dates.indexOf(value) > -1} onChange={handleCheckboxChange} value={value} />}
                                            label={label}
                                          />)
                                      })
                                    }
